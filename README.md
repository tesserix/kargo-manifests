...
| `kargo-tesserix-home` | live, auto-promoting | `^main-[a-f0-9]{7,12}$` | `NewestBuild` | 1 App (`company` — the tesserix-home Next.js site at tesserix.app), single-arch Docker v2 manifest, no `platform:` filter. Image repo is `tesserix/tesserix-home`; the in-cluster Deployment is named `company`. |
